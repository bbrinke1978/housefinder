...
/**
 * Sevier County scrape pipeline orchestrator.
 *
 * Runs at 5:30 AM Mountain Time (staggered after Emery at 5:15).
 * Parses annual delinquent tax PDF, upserts properties with tax_lien signals,
 * then scores and sends alerts.
 */
async function sevierScrapeHandler(
  myTimer: Timer,
  context: InvocationContext
): Promise<void> {
  const startTime = Date.now();
  context.log("Sevier County scrape pipeline starting...");
  ...
  try {
    ...
    const configKey = "sevier.delinquent.lastParsedYear";
    ...
    if (lastParsedYear === currentYear) {
      context.log(
        `[sevier] PDF already parsed for ${currentYear}, skipping`
      );
    } else {
      const records = await parsePdfDelinquent(sevierConfig);
      delinquentResult = await upsertFromDelinquent(records, "sevier");
      ...
      if (records.length > 0) {
        await db
          ...
          .values({
            ...
            value: currentYear,
            description:
              "Last year the Sevier County delinquent PDF was parsed",
          })
          .onConflictDoUpdate({
      ...
      context.log(
        `Sevier delinquent PDF: parsed ${records.length} records, upserted ${delinquentResult.upserted}, signals ${delinquentResult.signals}`
      );
    }
  } catch (err) {
    context.error("Sevier delinquent PDF parser failed", err);
    await updateScrapeHealth({
      county: "sevier",
      source: "delinquent-pdf",
      resultCount: 0,
  ...
  context.log("Sevier County scrape complete", {
...
// Schedule: 5:30 AM daily -- staggered after Emery County's 5:15 AM
