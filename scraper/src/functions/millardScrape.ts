...
/**
 * Millard County scrape pipeline orchestrator.
 *
 * Runs at 6:00 AM Mountain Time (staggered after Juab at 5:45).
 * Parses annual delinquent tax PDF, upserts properties with tax_lien signals,
 * then scores and sends alerts.
 */
async function millardScrapeHandler(
  myTimer: Timer,
  context: InvocationContext
): Promise<void> {
  const startTime = Date.now();
  context.log("Millard County scrape pipeline starting...");
  ...
  try {
    ...
    const configKey = "millard.delinquent.lastParsedYear";
    ...
    if (lastParsedYear === currentYear) {
      context.log(
        `[millard] PDF already parsed for ${currentYear}, skipping`
      );
    } else {
      const records = await parsePdfDelinquent(millardConfig);
      delinquentResult = await upsertFromDelinquent(records, "millard");
      ...
      if (records.length > 0) {
        await db
          ...
          .values({
            ...
            value: currentYear,
            description:
              "Last year the Millard County delinquent PDF was parsed",
          })
          .onConflictDoUpdate({
      ...
      context.log(
        `Millard delinquent PDF: parsed ${records.length} records, upserted ${delinquentResult.upserted}, signals ${delinquentResult.signals}`
      );
    }
  } catch (err) {
    context.error("Millard delinquent PDF parser failed", err);
    await updateScrapeHealth({
      county: "millard",
      source: "delinquent-pdf",
      resultCount: 0,
  ...
  // Step 4: Health check
  try {
    await checkHealthAlert("millard");
  } catch (err) {
  ...
  context.log("Millard County scrape complete", {
...
// Schedule: 6:00 AM daily -- staggered after Juab County's 5:45 AM
...
app.timer("millard-scrape", {
  schedule: "0 0 6 * * *",
  runOnStartup: false,
  handler: millardScrapeHandler,
});

export { millardScrapeHandler };
