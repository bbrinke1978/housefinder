 ...
 * SaltLake_County_Addresses layer. The allowlist is the source of truth for
 * which SLC parcels are actually in 84116 — the only reliable filter since
 * Utah Legals notices don't carry property zips reliably and "city" is the
 * law-firm jurisdiction, not the property's neighborhood.
 *
 * File ships with the scraper deployment. Re-generate annually (or after
 * significant SLC subdivision activity) with the build script.
 */
...
function getRoseParkAllowlist(): Set<string> {
  ...
  console.warn(
    `[utah-legals] WARNING: Rose Park parcel allowlist not found in any candidate path. SLC notices will all be excluded. Run scraper/src/scripts/build-rose-park-parcel-allowlist.ts to generate.`
  );
  ...
  try {
    ...
    while (hasNextPage && pageNum < 20) {
      ...
      for (const result of pageResults) {
        ...
        // RP-11 (final 2026-04-26): For Salt Lake County notices, filter on the
        // 84116 parcel-ID ALLOWLIST built by build-rose-park-parcel-allowlist.ts
        // from UGRC's SaltLake_County_Addresses layer. This is the only reliable
        // geographic filter:
        //   - Utah Legals notice city = law-firm jurisdiction (always "Salt Lake City"), useless
        //   - Snippet rarely contains the property zip code (300-char truncation)
        //   - UGRC Address Points has a definitive ParcelID + ZipCode mapping
        //
        // Notices whose extracted parcel ID isn't in the 84116 allowlist are skipped.
        // Notices whose parcel ID can't be extracted from the snippet are also skipped
        // (we can't confirm 84116, so we don't tag).
        let propertyZip: string | undefined;
        if (countyLower === 'salt lake') {
          const allowlist = getRoseParkAllowlist();
          ...
          if (!allowlist.has(normalized)) {
            console.log(
              `${tag} SLC notice excluded (parcel ${rawParcelId} -> ${normalized} not in 84116 allowlist) (ID: ${result.noticeId})`
            );
            continue;
          }
          console.log(
            `${tag} SLC notice ACCEPTED — 84116 parcel ${rawParcelId} matched allowlist (ID: ${result.noticeId})`
          );
          propertyZip = '84116';
        }
        ...
        const notice: UtahLegalsNotice = {
          ...
          zip: propertyZip,  // RP-11: '84116' only for SLC notices that passed the explicit zip-extraction filter above
        };
