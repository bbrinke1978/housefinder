...
export function FloorPlanSketch({
  ...
  const stageHeight = typeof window !== "undefined" && window.innerWidth < 640 ? 400 : 600;
  ...
  function buildGrid() {
    ...
    const rows = Math.ceil(stageHeight / GRID_SNAP) + 2;
  ...
  function handleDialogSave(
    ...
    if (editingRoom) {
      ...
      // Adding new room — place near top-left visible area
  ...
  // --- Transform end (resize with scale normalization) ---
  function handleTransformEnd(
    ...
    // Reset scale to 1 after reading
    node.scaleX(1);
    node.scaleY(1);
